    ...
    for fname in fileList:
        ...
        with io.open(filePath,'rb') as image_file:
            ...
            for text in text:
                ...
                with open (secondPath, 'a') as f:
                    f.write('Text Detected: %s, ' % textDesc)

            for logo in logos:
                print(logo.description)
                logoDesc = logo.description
                thirdPath = '/users/mattvukojevic/documents/github/targeted-offers/profiles/%s/%s-logos.txt' % (dirName[2:], dirName[2:])
                with open (thirdPath, 'a') as f:
                    f.write('Logo Detected: %s, ' % logoDesc)

            for landmark in landmarks:
                print(landmark.description)
                landmarkDesc = landmark.description
                fourthPath = '/users/mattvukojevic/documents/github/targeted-offers/profiles/%s/%s-landmarks.txt' % (dirName[2:], dirName[2:])
                with open (fourthPath, 'a') as f:
